...
namespace Trie
{
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Dictionary<string, string> dictionary = JsonSerializer.Deserialize<Dictionary<string, string>>(File.ReadAllText("dictionary.json"));
            ...
            while(true)
            {
                ...
                Console.WriteLine();
                List<string> stuff = tree.FindMatchingPrefixes(target);
                if (dictionary.ContainsKey(target))
                {
                    stuff.Insert(0, dictionary[target]);
                }
